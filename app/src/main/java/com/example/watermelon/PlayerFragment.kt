...
class PlayerFragment: Fragment(R.layout.fragment_player) {
    ...
    private val updateSeeokRunnalbe = Runnable {
            updateSeek()
    }
    ...
    private fun updateSeek() {
        ...
        view?.removeCallbacks(updateSeeokRunnalbe)
        if (state != Player.STATE_IDLE && state != Player.STATE_ENDED) {
            view?.postDelayed(updateSeeokRunnalbe, 1000)
        }
    }
    ...
    private fun getVideoListFromServer() {
        // HandShake 시간 재설정 -> 왜 why? -> 시간이 짧아 api를 받아오지 못했음
